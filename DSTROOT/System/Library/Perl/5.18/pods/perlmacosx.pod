...
So out of the box, the default version of perl is ${ENV_PERL_BASE_VERSION}.
...
system-wide basis.
The alternate version of perl that is provided is ${ENV_VERSION_ALT}.

Users can select the alternate version
by simply running the following command:

    % defaults write com.apple.versioner.perl Version ${ENV_VERSION_ALT}

Subsequent invocations of '/usr/bin/perl' will then use the ${ENV_VERSION_ALT} version.
